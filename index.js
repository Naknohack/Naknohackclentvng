...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'masoi') {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('Nhấn nút bên dưới để tham gia! Trò chơi sẽ bắt đầu sau **1 phút**.')
        ...
        if (interaction.isButton()) {
            ...
            if (id.startsWith('masoi_panel_')) {
                ...
                if (userRole === 'Sói') {
                    ...
                    skillText = "Bạn là PHÙ THỦY. Bạn có bình độc để giết người, và bình cứu để bảo vệ.";
                    const ptOptions = [...options, { label: '🧪 Bỏ qua (Không làm gì)', value: 'skip' }];
                    components.push(new ActionRowBuilder().addComponents(
                        new StringSelectMenuBuilder().setCustomId(`witch_poison_${guildId}`).setPlaceholder('Tạt bình độc ai?').addOptions(ptOptions)
                    ));
            ...
            if (id.includes('_')) {
                ...
                if (['wolf', 'seer', 'guard', 'witch', 'cupid'].includes(action)) {
                    ...
                    try {
                        if (action === 'wolf') {
                        ...
                        } else if (action === 'witch' && parts[1] === 'poison') {
                            ...
                            if (targetId === 'skip') {
                                ...
                                await interaction.update({ content: `🧪 Bạn đã tạt bình độc vào <@${targetId}>!`, embeds: [], components: [] });
                            }
...
function processNight(guildId, game) {
    ...
    for (const [target, count] of Object.entries(game.nightActions.wolfVotes)) {
        if (count > highestVote) { highestVote = count; killedByWolf = target; }
    }

    if (killedByWolf && game.nightActions.guardTarget !== killedByWolf) {
    ...
    if (pt && pt !== 'skip' && game.playerRoles[pt].alive) {
...
function checkWinCondition(guildId, game) {
    ...
    if (winningTeam) {
        const winEmbed = new EmbedBuilder()
            ...
            .setDescription(`Phe chiến thắng: **${winningTeam}**\n\nMỗi người sống sót thuộc phe thắng nhận được **100,000 VNĐ** 💸!`)
        ...
        for (const [id, data] of Object.entries(game.playerRoles)) {
            ...
            if ((winningTeam === 'Dân Làng' && !isWolf) || (winningTeam === 'Ma Sói' && isWolf)) {
